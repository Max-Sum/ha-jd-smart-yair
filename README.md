# JD Smart Yair AC for Home Assistant

Unofficial Home Assistant custom integration for Yair / Yangzi air conditioners controlled through the JD Smart / JD Xiaojia app.

This repository packages the JD Smart Yair air-conditioner protocol as a Home Assistant custom integration. It is not affiliated with JD.com, JD Smart, JD Xiaojia, Yair / Yangzi, or Home Assistant.

[简体中文 README](README_zh-Hans.md)

## Purpose

This repository is for users who own a Yair / Yangzi air conditioner controlled through the JD Smart / JD Xiaojia app and can extract session values from a local app traffic capture.
...
```text
https://github.com/Max-Sum/ha-jd-smart-yair
```
...
Optional. The full `Cookie` request header from the captured app request. Leave it
empty if the captured app `api.smart.jd.com` request does not include a
`Cookie` header.
